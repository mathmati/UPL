#!/usr/bin/env python3
"""Room bookings multi-user app - v1 (Python 3 stdlib only)."""
...
DB_PATH = os.environ.get("DB_PATH") or os.environ.get("MIURA_DB") or "bookings_v1.db"
...
<h2>All bookings</h2>
<table id="all-table">
  <thead><tr><th>Slot</th><th>Purpose</th><th>Owner</th><th>Actions</th></tr></thead>
  <tbody></tbody>
</table>

<h2>My bookings</h2>
<table id="my-table">
  <thead><tr><th>Slot</th><th>Purpose</th><th>Actions</th></tr></thead>
  <tbody></tbody>
</table>
...
async function book() {
  ...
  try {
    await api('/api/book', {method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({slot: document.getElementById('book-slot').value, purpose: document.getElementById('book-purpose').value})});
...
async function loadAll() {
  ...
  try {
    ...
    for (const b of all) {
      ...
      tr.innerHTML = `<td>${b.slot}</td><td>${b.purpose}</td><td>${b.owner_id}</td><td></td>`;
    ...
    for (const b of mine) {
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${b.slot}</td><td>${b.purpose}</td><td></td>`;
      const td = tr.lastElementChild;
      const editBtn = document.createElement('button');
      editBtn.textContent = 'Edit';
      editBtn.onclick = () => changePurpose(b.id);

...
def claim_public(row):
    return {
        ...
        "category": row["category"] if "category" in row.keys() else DEFAULT_CATEGORY,
    }
...
<div id="app-section" style="display:none">
  ...
  <h2>My claims</h2>
  <table id="my-table"><thead><tr><th>Reference</th><th>Amount</th><th>Category</th><th>Approved</th><th>Actions</th></tr></thead><tbody></tbody></table>

  <h2>All claims</h2>
  <table id="all-table"><thead><tr><th>Reference</th><th>Amount</th><th>Category</th><th>Approved</th><th>Owner</th><th>Actions</th></tr></thead><tbody></tbody></table>
</div>
...
async function submitClaim() {
  try {
    ...
    await api('/api/submit_claim', {method:'POST', body: JSON.stringify({reference, amount, category})});
    ...
    def _handle_submit_claim(self):
        ...
        try:
            ...
            if not isinstance(category, str) or not category.strip():
                raise ApiError(400, "category must be a non-empty string")
            ...
            created_at = now_iso()
            cur = conn.execute(
                "INSERT INTO claims (reference, amount, approved, owner_id, created_at, category) "
                "VALUES (?, ?, 0, ?, ?, ?)",
                (reference, amount, user["id"], created_at, category),
            )
            conn.commit()

#!/usr/bin/env python3
"""Snippet locker - multi-user shared code-snippet locker (v2).

v2 adds a `language` field (text, default "text") to Snippet.
...
<fieldset id="saveBox" style="display:none">
  ...
  <label>Language <input id="snipLanguage" placeholder="text" value="text"></label>
...
async function saveSnippet() {
  ...
  const language = qs('snipLanguage').value || 'text';
  const r = await api('POST', '/api/save_snippet', { title, body, language });
  if (r.status === 200) {
    qs('snipTitle').value = '';
    qs('snipBody').value = '';
    qs('snipLanguage').value = 'text';
    loadList();
  } else {
...
async function loadList() {
  ...
  for (const s of r.data) {
    ...
    div.innerHTML = `
      <h4>${escapeHtml(s.title)} <span class="meta">[${escapeHtml(s.language || 'text')}]</span></h4>
        ...
        try:
            ...
            with _db_lock:
                ...
                if dup:
                    ...
                    return
                cur = conn.execute(
                    "INSERT INTO snippets (title, body, owner_id, language) VALUES (?, ?, ?, ?)",
                    (title, body, user["id"], language),
                )
                conn.commit()

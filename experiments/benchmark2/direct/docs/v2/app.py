#!/usr/bin/env python3
"""Shared docs — multi-user document index app (v2, adds Doc.archived). Python 3 stdlib only."""
...
def init_db():
    with db_lock:
        ...
        conn.commit()
        # Defensive: if an existing v2 db was created before this column
        # existed (shouldn't happen via init_db, but guards hand-migrated
        # dbs opened directly by this app), add it if missing.
        cols = [r["name"] for r in conn.execute("PRAGMA table_info(docs)").fetchall()]
...
li.archived { opacity: 0.55; text-decoration: line-through; }
.err { color: red; }
</style>
</head>
<body>
...
async function toggleArchive(id, archived) {
  await api("POST", "/api/rename_doc", { id, archived: !archived });
  refresh();
}
...
function renderList(el, docs) {
  ...
  for (const d of docs) {
    ...
    if (d.archived) li.className = "archived";
    li.textContent = `[${d.id}] ${d.slug} — ${d.title} (owner ${d.owner_id}) ${d.archived ? "[archived] " : ""}`;
    const rb = document.createElement("button");
    rb.textContent = "rename";
    rb.onclick = () => renameDoc(d.id);
    const ab = document.createElement("button");
    ab.textContent = d.archived ? "unarchive" : "archive";
    ab.onclick = () => toggleArchive(d.id, d.archived);
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _create_doc(self, data):
        ...
        if not (1 <= len(title) <= 150):
            ...
            return
        archived = _as_bool(data.get("archived"), default=False)
        if archived is None:
            self._send_json(400, {"error": "invalid archived"})
            return
        with db_lock:
            ...
            if existing:
                ...
                return
            cur = conn.execute(
                "INSERT INTO docs (slug, title, owner_id, created_at, archived) VALUES (?,?,?,?,?)",
                (slug, title, user["id"], time.time(), int(archived)),
            )
            conn.commit()
    ...
    def _rename_doc(self, data):
        ...
        with db_lock:
            ...
            if has_archived:
                archived_val = _as_bool(data.get("archived"))
                if archived_val is None:
                    self._send_json(400, {"error": "invalid archived"})
                    return
                new_archived = int(archived_val)
            conn.execute(
                "UPDATE docs SET title=?, archived=? WHERE id=?",
                (new_title, new_archived, doc_id),
            )
            conn.commit()
            row = conn.execute("SELECT * FROM docs WHERE id=?", (doc_id,)).fetchone()
        self._send_json(200, doc_to_dict(row))

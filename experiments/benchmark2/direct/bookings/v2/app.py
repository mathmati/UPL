#!/usr/bin/env python3
"""Room bookings — multi-user booking board (v2, adds `attendees` field).
...
def action_book(conn, user, payload):
    ...
    attendees = payload.get("attendees", 1)
    if attendees is None:
        attendees = 1
    if isinstance(attendees, str) and attendees.strip().lstrip("-").isdigit():
    ...
    cur = conn.execute(
        "INSERT INTO bookings (slot, purpose, owner_id, created_at, attendees) VALUES (?, ?, ?, ?, ?)",
        (slot, purpose, user["id"], time.time(), attendees),
    )
    conn.commit()
...
<fieldset id="book-box">
  ...
  <input id="attendees" type="number" min="1" value="1" placeholder="attendees">
  <button onclick="book()">Book</button>
  <div class="err" id="book-err"></div>
</fieldset>

<h2>All bookings</h2>
<table id="all-table"><thead><tr><th>slot</th><th>purpose</th><th>attendees</th><th>owner</th><th></th></tr></thead><tbody></tbody></table>

<h2>My bookings</h2>
<table id="my-table"><thead><tr><th>slot</th><th>purpose</th><th>attendees</th><th></th></tr></thead><tbody></tbody></table>
...
function renderTable(tableId, rows, showOwner) {
  ...
  for (const b of rows) {
    ...
    let html = '<td>' + b.slot + '</td><td>' + b.purpose + '</td><td>' + b.attendees + '</td>';
...
async function book() {
  ...
  const attendees = parseInt(document.getElementById('attendees').value, 10) || 1;
  const r = await api('/api/book', { method: 'POST', body: JSON.stringify({ slot, purpose, attendees }) });
